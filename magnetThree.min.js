import*as THREE from 'https://threejs.org/build/three.module.js';import TWEEN from 'https://cdn.jsdelivr.net/npm/@tweenjs/tween.js@18.5.0/dist/tween.esm.js';import{OrbitControls}from 'https://threejs.org/examples/jsm/controls/OrbitControls.js';import{STLLoader}from 'https://threejs.org/examples/jsm/loaders/STLLoader.js';import{EffectComposer}from 'https://threejs.org/examples/jsm/postprocessing/EffectComposer.js';import{RenderPass}from 'https://threejs.org/examples/jsm/postprocessing/RenderPass.js';import{MaskPass,ClearMaskPass}from 'https://threejs.org/examples/jsm/postprocessing/MaskPass.js';import{ShaderPass}from 'https://threejs.org/examples/jsm/postprocessing/ShaderPass.js';import{BloomPass}from 'https://threejs.org/examples/jsm/postprocessing/BloomPass.js';import{ClearPass}from 'https://threejs.org/examples/jsm/postprocessing/ClearPass.js';import{CopyShader}from 'https://threejs.org/examples/jsm/shaders/CopyShader.js';import{FXAAShader}from 'https://threejs.org/examples/jsm/shaders/FXAAShader.js';const maxParticleCount=500;let particleCount=300;const rad=350;const rHalf=rad/2;var particleStrengthGroup;var particleLineNetworkGroup,particleNetworkGroup,particleDataNetworkGroup,logoNetworkGroup;let particlesData=[];let positions,colors;let particles;let pointCloud;let particlePositions;let linesMesh;let camera,renderer,scene,sceneBg;var particleNetworkCount=60;let canvasDom;var frameNetworkCount=238;var nAngle=0;let composer;initThree("Three-container");initComposer();animate();function initComposer(){const width=canvasDom.offsetWidth;const height=canvasDom.offsetHeight;const clearPass=new ClearPass();const clearMaskPass=new ClearMaskPass();const maskPass=new MaskPass(logoNetworkGroup,camera);const renderPass=new RenderPass(particleStrengthGroup,camera);renderPass.clear=!1;const renderBg=new RenderPass(sceneBg,camera);const outputPass=new ShaderPass(CopyShader);outputPass.renderToScreen=!0;const fxaaPass=new ShaderPass(FXAAShader);const pixelRatio=renderer.getPixelRatio();fxaaPass.material.uniforms.resolution.value.x=1/(width*pixelRatio);fxaaPass.material.uniforms.resolution.value.y=1/(height*pixelRatio);const parameters={minFilter:THREE.LinearFilter,magFilter:THREE.LinearFilter,format:THREE.RGBFormat,stencilBuffer:!0};const renderTarget=new THREE.WebGLRenderTarget(width,height,parameters);composer=new EffectComposer(renderer,renderTarget);composer.addPass(clearPass);composer.addPass(renderBg);composer.addPass(maskPass);composer.addPass(renderPass);composer.addPass(clearMaskPass);composer.addPass(fxaaPass);composer.addPass(outputPass)}
...
return mesh}
function getDistance(v1,v2){const dX=v1.x-v2.x;const dY=v1.y-v2.y;const dZ=v1.z-v2.z;const distance=Math.sqrt(dX*dX+dY*dY+dZ*dZ);const xCenter=(v1.x+v2.x)/2;const yCenter=(v1.y+v2.y)/2;if((xCenter>=-160&&xCenter<=160)&&(yCenter>=-60&&yCenter<=60))
return distance*2;return distance}
function onWindowResize(){const width=canvasDom.offsetWidth;const height=canvasDom.offsetHeight
camera.aspect=width/height;camera.updateProjectionMatrix();renderer.setSize(width,height)}
function initThree(id,width,height){console.log(id);if(id==undefined){canvasDom=document.body}else canvasDom=document.getElementById(id);console.log(canvasDom);if(width==undefined){width=canvasDom.offsetWidth}
if(height==undefined){height=canvasDom.offsetHeight;if(height==0){height=1000}}
console.log(width,height);const aspect=width/height;camera=new THREE.PerspectiveCamera(50,aspect,300,3000);camera.position.x=0;camera.position.y=-300;camera.position.z=1000;camera.position.x=-200;camera.lookAt(0,0,0);sceneBg=new THREE.Scene();scene=new THREE.Scene();sceneBg.background=new THREE.Color(0xf8f8f8);renderer=new THREE.WebGLRenderer({antialias:!0});renderer.setPixelRatio(window.devicePixelRatio);renderer.setSize(width,height);renderer.autoClear=!1;renderer.shadowMap.enable=!0;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.interpolateneMapping=THREE.ACESFilmicToneMapping;canvasDom.appendChild(renderer.domElement);const controls=new OrbitControls(camera,renderer.domElement);controls.enableZoom=!1;controls.enablePan=!1;controls.mouseButtons={}
particleLineNetworkGroup=new THREE.Group();particleNetworkGroup=new THREE.Group();particleDataNetworkGroup=new THREE.Group();logoNetworkGroup=new THREE.Group();particleStrengthGroup=new THREE.Group();for(var i=0;i<particleNetworkCount;i++){const rho=(Math.PI)*6*i/particleNetworkCount+(Math.random()*Math.PI/6-Math.PI/12);const theta=(Math.PI)*8*i/particleNetworkCount+(Math.random()*Math.PI/6-Math.PI/12);const r=(Math.random()*200+300);const x=0.8*(r*Math.sin(rho)*Math.cos(theta));const y=0.8*(r*Math.sin(rho)*Math.sin(theta));const z=0.8*(r*Math.cos(rho));const size=Math.random()*1.5+2;const particle=createSphere(undefined,particleNetworkGroup,undefined,new THREE.Vector3(x,y,z),new THREE.Euler(0,0,0),size,0x8f8f8f,);const particleData=createSphere(undefined,particleDataNetworkGroup,undefined,new THREE.Vector3(x,y,z),new THREE.Euler(0,0,0),2,0xafafaf,)}
...
linesMesh.geometry.setDrawRange(0,numConnected*2);linesMesh.geometry.attributes.position.needsUpdate=!0;linesMesh.geometry.attributes.color.needsUpdate=!0;pointCloud.geometry.attributes.position.needsUpdate=!0;const t=Date.now()*0.001;particleStrengthGroup.rotation.y=t*0.1}
